...
import { createBehavior, deleteBehavior, getAllBehaviors } from '../behavior-antecedent-intervention-categories/store/actions';
...
const BehaviorAntecedentIntervention = () => {
  ...
  const behaviorStore=useSelector(state=>state.behavior.behaviors)
  ...
  useEffect(() => {
    ...
    for(let i = 0; i < data.length; i++){
      if(data[i].type === "Behavior"){
        tempBehavior.push(data[i])
      }
      if(data[i].type === "Antecedent"){
        tempAntecedent.push(data[i])
      }
      if(data[i].type === "Intervention"){
        tempIntervention.push(data[i])
      }
    }
  ...
  const onDeleteBehavior = () => {
    ...
    dispatch(deleteBehavior(deleteEntryDataB))
  }
  ...
  const onDeleteAntecedent = () => {
    ...
    //     console.log((err))
    // })
    dispatch(deleteBehavior(deleteEntryDataA))
  ...
  const onDeleteIntervention = () => {
    ...
    //     console.log((err))
    // })
    dispatch(deleteBehavior(deleteEntryDataI))
  }
  ...
  const currentTableDataBehavior = useMemo(() => {
    let behaviorListFromBackend = behaviorStore.filter((b) => b.type === 'Behavior')
  ...
  }, [currentPageB, behaviorStore, PageSizeB, behaviorListStatic]);

  const currentTableDataAntecedent = useMemo(() => {
    let behaviorListFromBackend = behaviorStore.filter((b) => b.type === 'Antecedents')
    if (behaviorListFromBackend.length <= PageSizeA) {
      setCurrentPageA(1)
    }
    let tempdata = behaviorListFromBackend 
    ...
    return tempdata.slice(firstPageIndex, lastPageIndex);
  }, [currentPageA, antecedentList,behaviorStore, PageSizeA, antecedentListStatic])

  const currentTableDataIntervention = useMemo(() => {
    let behaviorListFromBackend = behaviorStore.filter((b) => b.type === 'Intervention')
    if (behaviorListFromBackend.length <= PageSizeI) {
      setCurrentPageI(1)
    }
    let tempdata = behaviorListFromBackend
    ...
    return tempdata.slice(firstPageIndex, lastPageIndex);
  }, [currentPageI, interventionList,behaviorStore, PageSizeI, interventionListStatic])
          ...
          <div className='fv-row fv-plugins-icon-container px-5 card'>
            <Formik
              ...
              onSubmit={(values, { setSubmitting, resetForm }) => {
                ...
                dispatch(createBehavior(values))
                resetForm()
                setSubmitting(false)
              }}
            >
      ...
      {/* Behavior List */}
      {
        showEditingFormB ?
          <div>
                        ...
                        <tbody>
                          {
                            currentTableDataBehavior.length > 0 ?
                              currentTableDataBehavior.map((behavior, index) => (
                                <Fragment key={index}>
                                  <BRow behavior={behavior} setEditEntryDataB={setEditEntryDataB}
                                    setDeleteEntryDataB={setDeleteEntryDataB} setShowEditingFormB={setShowEditingFormB} />
                                </Fragment>
                              )) :
                              <tr>
                                <td align="center" colSpan={12}><span className='text-gray-600 fs-2 fw-bolder'>No Behavior History Found</span></td>
                              </tr>
                          }
                        </tbody>
                      </table>
            ...
            <div className='modal fade' id="kt_modal_delete_behavior" tabIndex={-1} aria-hidden="true">
              {/*begin::Modal dialog*/}
                  ...
                  <div className='modal-body scroll-y mx-lg-5'>
                    <div className='d-flex justify-content-center'>
                      <button className='btn btn-danger me-3'
                        data-bs-dismiss="modal" aria-label="Close" onClick={onDeleteBehavior}>Confirm
                      </button>
          ...
          : <BEditing setEditEntryDataB={setEditEntryDataB} setShowEditingFormB={setShowEditingFormB}
            editEntryDataB={editEntryDataB} updateBehavior={updateBehavior} />
      }
      ...
      {/* ANTECEDENT LIST */}
      {
        showEditingFormA ?
          <div>
                        ...
                        <tbody>
                          {
                            currentTableDataAntecedent.length > 0 ?
                              currentTableDataAntecedent.map((antecedent, index) => (
                                <Fragment key={index}>
                                  <ARow antecedent={antecedent} setEditEntryDataA={setEditEntryDataA}
                                    setDeleteEntryDataA={setDeleteEntryDataA} setShowEditingFormA={setShowEditingFormA} />
                                </Fragment>
                              )) :
                              <tr>
                                <td align="center" colSpan={12}><span className='text-gray-600 fs-2 fw-bolder'>No Antecedent History Found</span></td>
                              </tr>
                          }
                        </tbody>
                      </table>
            ...
            <div className='modal fade' id="kt_modal_delete_antecedent" tabIndex={-1} aria-hidden="true">
              {/*begin::Modal dialog*/}
              <div className="modal-dialog modal-dialog-centered mw-600px mh-100px">
                {/*begin::Modal content*/}
                <div className="modal-content">
                  ...
                  <div className='modal-body scroll-y mx-lg-5'>
                    <div className='d-flex justify-content-center'>
                      <button className='btn btn-danger me-3'
                        data-bs-dismiss="modal" aria-label="Close" onClick={onDeleteAntecedent}>Confirm
                      </button>
                      <button className='btn btn-light me-3' data-bs-dismiss="modal"
                        aria-label="Close">Discard
                      </button>
          ...
          : <AEditing setEditEntryDataA={setEditEntryDataA} setShowEditingFormA={setShowEditingFormA}
            editEntryDataA={editEntryDataA} updateAntecedent={updateAntecedent} />
      }
      ...
      {/* INTERVENTION LIST */}
      {
        showEditingFormI ?
          <div>
                        ...
                        <tbody>
                          {
                            currentTableDataIntervention.length > 0 ?
                              currentTableDataIntervention.map((intervention, index) => (
                                <Fragment key={index}>
                                  <IRow intervention={intervention} setEditEntryDataI={setEditEntryDataI}
                                    setDeleteEntryDataI={setDeleteEntryDataI} setShowEditingFormI={setShowEditingFormI} />
                                </Fragment>
                              )) :
                              <tr>
                                <td align="center" colSpan={12}><span className='text-gray-600 fs-2 fw-bolder'>No Intervention History Found</span></td>
                              </tr>
                          }
                        </tbody>
                      </table>
            ...
            <div className='modal fade' id="kt_modal_delete_intervention" tabIndex={-1} aria-hidden="true">
              {/*begin::Modal dialog*/}
              <div className="modal-dialog modal-dialog-centered mw-600px mh-100px">
                {/*begin::Modal content*/}
                <div className="modal-content">
                  ...
                  <div className='modal-body scroll-y mx-lg-5'>
                    <div className='d-flex justify-content-center'>
                      <button className='btn btn-danger me-3'
                        data-bs-dismiss="modal" aria-label="Close" onClick={onDeleteIntervention}>Confirm
                      </button>
                      <button className='btn btn-light me-3' data-bs-dismiss="modal"
                        aria-label="Close">Discard
                      </button>
          ...
          : <IEditing setEditEntryDataI={setEditEntryDataI} setShowEditingFormI={setShowEditingFormI}
            editEntryDataI={editEntryDataI} updateIntervention={updateIntervention} />
      }
    </div >
  )
}
