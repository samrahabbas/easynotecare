...
import SharedContactListRow from './SharedContactListRow'
import SharedContactEditForm from './SharedContactEditForm'
...
const SharedContactList = () => {
    ...
    const [editSharedContact, setEditSharedContact] = useState({})
        ...
        axios.get('/shared/get/' + companyid)
            .then((res) => {
                ...
                for (let i = 0; i < tempvar.length; i++) {
                    ...
                    tempvar[i].primaryPhone = res.data[i].mailingInfo?.primaryPhone

                }
                setSharedContactStatic(tempvar)
                setSharedContacts(tempvar)
            })
    }, [showEditForm])
    ...
    const onDeleteSharedContact = () => {
        axios.delete('/shared/delete/' + deleteSharedContact)
            .then(() => {
                        ...
                        <div id='kt_content_container' className='container-xxl'>
                            {
                                showEditForm ?
                                                    ...
                                                    <tbody className='text-gray-600 fw-bold'>
                                                        {
                                                            ContactsPaginationData.length ? ContactsPaginationData.map((contact, index) => (
                                                                <Fragment key={index}>
                                                                    <SharedContactListRow contact={contact} setEditSharedContact={setEditSharedContact}
                                                                        setDeleteSharedContact={setDeleteSharedContact} setShowEditForm={setShowEditForm} />
                                                                </Fragment>
                                                            ))
                                                                :
                                                                <tr>
                                    ...
                                    :
                                    <SharedContactEditForm setShowEditForm={setShowEditForm} setEditSharedContact={setEditSharedContact} editSharedContact={editSharedContact} />
                            }
                        </div>
                    </div>
                </div>
                ...
                <div className="modal-dialog modal-dialog-centered mw-600px mh-100px">
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-header d-flex justify-content-center">
                            {/*begin::Modal title*/}
                            <h2 className="fw-bolder">ARE YOU SURE YOU WANT TO DELETE THIS SHARED CONTACT?</h2>
                            {/*end::Modal title*/}
                        </div>
                        {/*end::Modal header*/}
                        {/*begin::Modal body*/}
